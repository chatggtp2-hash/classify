...
export async function modifyDocxMetadataInPlace(
  filePath: string,
...
async function updateCoreProperties(zip: JSZip, classification: string): Promise<void> {
  const corePath = "docProps/core.xml";
  const coreXml = await zip.file(corePath)?.async("text");
  if (!coreXml) return; // Not fatal - custom properties still carry classification.
...
async function upsertCustomProperties(zip: JSZip, properties: Record<string, string>): Promise<void> {
  ...
  const existingProps = new Map<string, { pid: number; value: string }>();
  let maxPid = 2;
  ...
  const propertyXmlParts = [...existingProps.entries()].map(
    ([name, { pid, value }]) =>
      `<property fmtid="{D5CDD505-2E9C-101B-9397-08002B2CF9AE}" pid="${pid}" name="${escapeXml(
        name
